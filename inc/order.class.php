...
class PluginOrderOrder extends CommonDBTM
{
   ...
   public function showForm($ID, $options = []) {
      ...
      if ($canedit) {
         $objectName = autoName(
             $this->fields["name"], "name", ($template === "newcomp"),
      ...
      if ($canedit) {
         $objectOrder = autoName(
             $this->fields["num_order"], "num_order", ($template === "newcomp"),
             $this->getType(), $this->fields["entities_id"]
          );
      ...
      if ($canedit && !$this->checkIfDetailExists($ID)) {
         ...
         Supplier::dropdown(
          ...
          Ajax::updateItemOnSelectEvent(
             "dropdown_suppliers_id$rand", "show_contacts_id$rand",
         ...
         if ($this->fields['users_id']) {
            ...
            if ($user->getFromDB($this->fields['users_id'])) {
                $output = formatUserName(
                    $this->fields['users_id'], $user->fields['name'],
            ...
            try {
             ...
             if ($this->fields["entities_id"] != 0) {
               ...
               if ($contact->getFromDB($this->fields["contacts_id"])) {
                   $output = formatUserName(
                     $contact->fields["id"], "", $contact->fields["name"],
   ...
   public static function updateBillState($ID) {
       $all_paid    = true;
       $order_items = getAllDatasFromTable(
   ...
   public static function install(Migration $migration) {
      ...
      if (!$DB->tableExists($table) && !$DB->tableExists("glpi_plugin_order")) {
          ...
          Crontask::Register(
              __CLASS__, 'computeLateOrders', HOUR_TIMESTAMP, [
              'param' => 24,
         ...
         $migration->changeField(
             $table, "FK_entities", "entities_id",
             "int(11) NOT NULL default 0"
         );
         $migration->changeField(
             $table, "recursive", "is_recursive",
         ...
         if ($DB->tableExists("glpi_plugin_order_budgets")) {
             ...
             $migration->changeField(
                 "glpi_plugin_order_budgets", "FK_entities", "entities_id",
                 "int(11) NOT NULL default '0'"
             );
             $migration->changeField(
                 "glpi_plugin_order_budgets", "FK_budget", "budgets_id",
                 "int(11) NOT NULL default '0'"
             );
             $migration->changeField(
                 "glpi_plugin_order_budgets", "comments", "comment",
                 "text collate utf8_unicode_ci"
             );
             $migration->changeField(
                 "glpi_plugin_order_budgets", "deleted", "is_deleted",
                 "tinyint(1) NOT NULL default '0'"
             );
             $migration->changeField(
                 "glpi_plugin_order_budgets", "startdate", "start_date",
                 "date default NULL"
             );
             $migration->changeField(
                 "glpi_plugin_order_budgets", "enddate", "end_date",
                 "date default NULL"
             );
             $migration->changeField(
                 "glpi_plugin_order_budgets", "value", "value",
         ...
         //1.3.0
         $migration->addField(
             $table, "plugin_order_ordertypes_id",
             "int (11) NOT NULL default '0' COMMENT 'RELATION to glpi_plugin_order_ordertypes (id)'"
         );
         $migration->migrationOneTable($table);

         //1.4.0
         if ($migration->changeField(
             "glpi_plugin_order_orders", "states_id",
             "plugin_order_orderstates_id", "int(11) NOT NULL default 1"
         )
         ) {
             $migration->migrationOneTable($table);
