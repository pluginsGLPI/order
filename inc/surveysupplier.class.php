...
class PluginOrderSurveySupplier extends CommonDBChild {
   ...
   function getFromDBByOrder($plugin_order_orders_id) {
	...
	function getTotalNotation($plugin_order_orders_id) {
      ...
      $query = "SELECT (`answer1` + `answer2` + `answer3` + `answer4` + `answer5`) AS total FROM `".$this->getTable()."` " .
				"WHERE `plugin_order_orders_id` = '".$plugin_order_orders_id."' ";
	...
	function showOrderSupplierSurvey($target, $ID) {
      ...
      echo "<form method='post' name='show_suppliersurvey$rand' id='show_suppliersurvey$rand' action=\"$target\">";
      ...
      if ($DB->numrows($result) > 0) {
         ...
         if ($candelete)
         {
            ...
            echo "<tr><td><img src=\"".$CFG_GLPI["root_doc"]."/pics/arrow-left.png\" alt=''></td><td class='center'><a onclick= \"if ( markCheckboxes('show_suppliersurvey$rand') ) return false;\" href='".$_SERVER['PHP_SELF']."?id=$ID&amp;select=all'>".$LANG['buttons'][18]."</a></td>";

            echo "<td>/</td><td class='center'><a onclick= \"if ( unMarkCheckboxes('show_suppliersurvey$rand') ) return false;\" href='".$_SERVER['PHP_SELF']."?id=$ID&amp;select=none'>".$LANG['buttons'][19]."</a>";
